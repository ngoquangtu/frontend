...
const CourseDetails = () => {
  ...
  useEffect(() => {
    ...
    const fetchUserRating = async () => {
      if (isLoggedIn) {
        try {
          const response = await fetch(`https://${process.env.REACT_APP_LOCALHOST}:${process.env.PORT}/api/courses/rating/${id}`, {
            method: 'GET',
            headers: {
  ...
  const fetchCourseDetails = async () => {
    try {
      const response = await fetch(`https://${process.env.REACT_APP_LOCALHOST}:${process.env.PORT}/api/courses/${id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + localStorage.getItem('token')
  ...
  const fetchComments = async () => {
    try {
      const response = await fetch(`https://${process.env.REACT_APP_LOCALHOST}:${process.env.PORT}/api/courses/comment/${id}`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + localStorage.getItem('token')
  ...
  const addRating = async () => {
    try {
      const response = await fetch(`https://${process.env.REACT_APP_LOCALHOST}:${process.env.REACT_APP_PORT}/api/courses/rating/${id}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer ' + localStorage.getItem('token')
